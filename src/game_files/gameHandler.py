    ...
    def check_collision(self, agent:mayGameEntity) -> None:
        ...
        for ent in self.gameObjects:
            ...
            if any(i in agentXRange for i in entXRange) and any(i in agentYRange for i in entYRange):
                ...
                if isinstance(agent, mayGameEntity):
                    if ent.name == 'coin':
                        ## only allow player to get coins
                        if isinstance(agent, player):
                            ent.isAlive = False
                            self.player.score += ent.incScore
                            continue
                        ## dont collide with coins (causes issues with NPCs (they go flying))
                        ## comment this if youd love to see that (not recommended)
                        continue
                    
                    elif ent.name == 'healthKit':
                        ent.isAlive = False
                        agent.health += ent.health_inc
                        continue
